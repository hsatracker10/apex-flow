...
class AIEnhancementService: ObservableObject {
    ...
    /// Strips XML/HTML closing tags and common prompt-injection patterns from
    /// untrusted content before it is injected into an LLM prompt.
    /// This prevents content like `</TRANSCRIPT>\nIgnore prior instructions`
    /// from breaking out of its structured context section.
    private func sanitizeForPrompt(_ input: String) -> String {
        // Remove any closing tags that match our prompt structure markers
    ...
    private func getSystemMessage(for mode: EnhancementPrompt) async -> String {
        ...
        if AXIsProcessTrusted() {
            if let selectedText = await SelectedTextService.fetchSelectedText(), !selectedText.isEmpty {
                let safe = sanitizeForPrompt(selectedText)
        ...
        let clipboardContext = if useClipboardContext,
            ...
            "\n\n<CLIPBOARD_CONTEXT>\n\(sanitizeForPrompt(clipboardText))\n</CLIPBOARD_CONTEXT>"
        } else {
            ""
        }
        ...
        let screenCaptureContext = if useScreenCaptureContext,
            ...
            "\n\n<CURRENT_WINDOW_CONTEXT>\n\(sanitizeForPrompt(capturedText))\n</CURRENT_WINDOW_CONTEXT>"
        } else {
            ""
        }
